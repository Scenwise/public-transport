...
const jsonInterfaceConverter = (
    ...
    const mapShapeIdStops = new Map<number, ShapeIdStops>();
    const mapStopIds = new Map<number, Stop>();
    try {
...
const jsonInterfaceConverterRoutes = (
    ...
    try {
        data.features.forEach((element) => {
            ...
            if (
                filters !== undefined ||
                (filterAgencies.size > 0 && !filterAgencies.has(property.agency_id)) ||
                (filterModalities.size > 0 && !filterModalities.has(property.vehicle_type)) ||
                (filterLineNumber.size > 0 && !filterLineNumber.has(property.line_number))
            ) {
                return;
            }
