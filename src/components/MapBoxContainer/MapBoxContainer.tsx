    ...
    useEffect(() => {
        ...
        map.on('load', async function () {
            ...
            const [routeLayer, agenciesSet, modalitiesSet] = loadLineStringLayer(
                map,
                data,
                { Agency: new Set<string>(), 'Vehicle Type': new Set<string>(), 'Line Number': new Set<string>() },
                offset as number,
            );
        ...
        const selectFeaturesFunc = (e: mapboxgl.MapMouseEvent & mapboxgl.EventData) => {
            const offset = 0.5;
            const mapLinePoint: { lines: GeoJsonProperties | undefined; points: GeoJsonProperties | undefined } = {
                lines: undefined,
                points: undefined,
            };
