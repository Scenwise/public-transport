...
const DropDownMenu = ({
    ...
    return (
        <>
            <div className={styles['drop-down-menu-title-div']}>
                <span onClick={() => setState(!openState)} className={styles['drop-down-menu-title-span']}>
                    {name}
                </span>
            </div>
            <div className={openState === true ? styles['open-filter-div'] : styles.filterDiv}>
                {openState === true ? dispalyOptions() : null}
            </div>
        </>
    );
};
...
const Option = ({
    ...
    return (
        <div
            className={
                containsDictionaryKey(ddMenuName) === false ? styles['options-div'] : styles['options-div-selected']
            }
            onClick={() => {
                addRemoveOption();
            }}
    ...
    return (
        <div
            ...
            onClick={() => {
                ...
                const [routeLayer, ,] = loadLineStringLayer(
                    map,
                    geoDataPTLines,
                    filterResult?.filterToPass,
                    offset as number,
                );
                if (routeLayer !== null && routeLayer !== undefined) {
                    setDisplayGeoDataPTLines(routeLayer);
