...
const TextInputFilter = ({
    ...
    const updateFilter = (lineNumber: string) => {
        ...
        if (geoDataPTLines !== undefined) {
            if (lineNumber === 'Remove') {
                ...
                dispatch(
                    allActions.setFilterAction.setFilterReducer(
                        filterResult?.dictionary as {
                            Agency: Set<string>;
                            'Vehicle Type': Set<string>;
    ...
    return (
        <div>
            <div className={styles.textInputNameDiv}>
                <span onClick={() => changeOpenCloseState(!openClose)} className={styles['text-input-name-span']}>
                    {name}
                </span>
            </div>
            {openClose === true ? (
                <div className={styles['text-input-div']}>
                    <div
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewLineNumber(e.target.value)}
                        className={styles['inner-divs']}
                    >
                        <input value={lineNumber} type={'text'}></input>
                    </div>
                    <div className={styles['inner-divs']}>
                        <button onClick={() => updateFilter(lineNumber)}>Filter</button>
                    </div>
                    <div className={styles['inner-divs']}>
                        <button onClick={() => updateFilter('Remove')}>Remove</button>
                    </div>
