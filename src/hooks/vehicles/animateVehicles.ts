...
const animateVehicles = (vehicleRoutePair: VehicleRoutePair, newPosition: Array<number>, map: mapboxgl.Map) => {

  // Reduce the multiline string into a single linestring for easier traversal
  const line = vehicleRoutePair.route.geometry.coordinates;

  // Find current and new positions on route and slice the route to that zone only
  const convertedLine = turf.featureCollection(line.map((x: Array<number>) => turf.point(x)) as turf.Feature<turf.Point, turf.Properties>[])
  const startPosition = turf.nearestPoint(vehicleRoutePair.marker.getLngLat().toArray(), convertedLine).geometry.coordinates
  ...
  const startIndex = line.findIndex((coord: Array<number>) => coord[0] === startPosition[0] && coord[1] === startPosition[1])
  const endIndex = line.findIndex((coord: Array<number>) => coord[0] === endPosition[0] && coord[1] === endPosition[1])
