...
export const useKV6Websocket = (map: mapboxgl.Map | null, routeTree: React.MutableRefObject<RBush<PTRouteIndex>>, routesData: PTRouteFeature[]): void => {
        ...
        for (const packet of packets) {
          ...
          if (vehicle.messageType === "ONROUTE" && vehicle.rdX !== -1 && vehicle.rdY !== -1 && vehicle.longitude && vehicle.latitude && map !== null && routesData !== null) {
            ...
            if (vehicleRoutePair !== undefined && vehicleRoutePair.vehicle.timestamp < vehicle.timestamp) {
              const correct = animateVehicles(vehicleRoutePair, [vehicle.longitude, vehicle.latitude], map)
              if(correct) {
